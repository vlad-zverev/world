...
- reusable country drill-downs for South Africa and Lesotho, with administrative boundaries, highlighted travel regions, detailed destinations and airports;
...
Selecting South Africa or Lesotho opens a country map at the same size as the Africa map. Destination markers open place cards with access, connectivity, difficulty, accessibility, 4×4 guidance, suggested time, nearby combinations, local imagery, sourced field notes and an editorial world-class score.
...
Destination trivia and reading links are maintained directly in `data/destination-field-notes.json`; its country and destination keys must match the two destination datasets.

Refresh local destination images from Wikipedia/Wikimedia:
...
python3 scripts/fetch_destination_images.py \
  data/lesotho-destinations.json \
  assets/places/ls
```
